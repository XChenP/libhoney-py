...
class Transmission:
    def __init__(
        ...
        if user_agent_addition:
            user_agent = (
                f"libhoney-py/{VERSION} {user_agent_addition} python/{python_version()}"
            )
        else:
    ...
    def _send_batch(self, destination, events):
        ...
        try:
            url = urljoin(
                urljoin(destination.api_host, "/1/batch/"), destination.dataset
            )
            payload = []
            for ev in events:
            ...
            for ev, status in zip(events, statuses):
                self._enqueue_response(
                    status.get("status"), "", status.get("error"), start, ev.metadata
                )
